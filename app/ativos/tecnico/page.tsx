...
import { 
  getTecnicoOrdens, updateOrdemServicoAtivo, getLoggedTenantInfo, getOrdensServicoAtivo
} from '../actions';
...
export default function TecnicoPage() {
  ...
  const loadWelcomeStats = async (user: any) => {
    ...
    try {
      ...
      const totalSold = wonProps.reduce((sum: number, p: any) => sum + (p.valor || 0), 0);
      setValorVendido(totalSold);
  ...
  if (showWelcome) {
    ...
    const isSalesRole = !isSomenteTecnico;
      ...
      <div className="fixed inset-0 z-50 h-[100dvh] w-screen overflow-hidden bg-gradient-to-br from-slate-950 via-slate-900 to-[#1B4D3E] text-white flex flex-col font-sans select-none">
        <div className="flex-1 flex flex-col justify-between items-center p-3.5 max-w-md mx-auto w-full h-full max-h-[100dvh]">
          ...
          <div className="flex flex-col items-center text-center pt-1.5 pb-2 w-full">
            <span className="text-[8px] font-extrabold text-emerald-350 uppercase tracking-widest bg-white/10 px-3 py-1 rounded-full border border-white/10 shadow-xs mb-2">
              {tenant?.nome || 'Slimpe'}
            </span>
            {currentUser?.avatarUrl ? (
              ...
              <div className="w-12 h-12 rounded-full bg-emerald-500/20 text-emerald-300 border-2 border-emerald-500/30 flex items-center justify-center font-black text-base shadow-md mb-2">
                {currentUser?.nome ? currentUser.nome.split(' ').map((n: string) => n[0]).slice(0, 2).join('').toUpperCase() : 'T'}
              </div>
            )}
            <h1 className="text-base sm:text-lg font-black tracking-tight leading-tight">Olá, {currentUser?.nome?.split(' ')[0] || 'Colaborador'}!</h1>
                ...
                <div className="relative flex flex-col items-center bg-white/5 backdrop-blur-md border border-white/10 p-4 rounded-2xl shadow-lg w-full max-w-xs mx-auto">
                  ...
                  <div className="relative w-full aspect-[100/68] max-w-[170px] flex items-center justify-center">
                    <svg viewBox="0 0 100 68" className="w-full h-auto">
                      <defs>
                        <linearGradient id="welcomeGaugeGradTech" x1="0%" y1="0%" x2="100%" y2="0%">
                        ...
                        <clipPath id="welcomeGaugeClipTech">
                          <path 
                      ...
                      <path 
                        d="M 10 50 A 40 40 0 0 1 90 50" 
                        fill="none" 
                        stroke="url(#welcomeGaugeGradTech)" 
                        strokeWidth="6" 
                        strokeLinecap="butt" 
                        strokeDasharray="11.216 1.5"
                        clipPath="url(#welcomeGaugeClipTech)" 
                      />
                      {/* Needle */}
                  ...
                  <div className="space-y-1.5 w-full max-w-xs">
                    ...
                    <div className="bg-white/5 border border-white/10 rounded-xl p-2 sm:p-2.5 flex items-center justify-between shadow-xs">
                      <div className="flex items-center gap-2">
                        ...
                        <div className="text-left">
                          <span className="text-[9px] font-bold text-slate-450 block leading-none mb-0.5">Serviços do Mês</span>
                    ...
                    <div className="bg-white/5 border border-white/10 rounded-xl p-2 sm:p-2.5 flex items-center justify-between shadow-xs">
                      <div className="flex items-center gap-2">
                        ...
                        <div className="text-left">
                          <span className="text-[9px] font-bold text-slate-450 block leading-none mb-0.5">Entregas do Mês</span>
                          <span className="text-xs font-black text-white leading-none">
                    ...
                    <div className="bg-gradient-to-r from-emerald-600/20 to-teal-500/20 border border-emerald-500/10 rounded-xl p-2 sm:p-2.5 flex items-center justify-between shadow-xs">
                      <div className="flex items-center gap-2">
                        ...
                        <div className="text-left">
                          <span className="text-[9px] font-bold text-slate-450 block leading-none mb-0.5">Valores Acumulados</span>
          ...
          <div className="w-full max-w-xs mx-auto pb-1.5 pt-1.5 flex flex-col gap-2">
            ...
            >
              {isSalesRole ? 'Acessar CRM (Comercial)' : 'Acessar Painel Técnico'}
            </button>
