const fs = require('fs');
const old = fs.readFileSync('old_fpv_utf8.tsx', 'utf8');

// Find the start
const startStr = '<div className={`print-slide-deck hidden ${viewMode === \'slide\' ? \'print:block\' : \'\'}`}>';
const startIndex = old.indexOf(startStr);

// Find the end: look for } from "lucide-react"; (wait, no)
// Look for the end of the print-slide-deck div.
// It ends right before `</div>\n      {/* Fim do Main Content */}`
const endStr = '</div>\n      {/* Fim do Main Content */}';
const endIndex = old.indexOf(endStr);
...
let printDeck = old.substring(startIndex, endIndex);
...
// Find the specific variables used in printDeck that might be undefined:
// `proposta` is available.
// `formatCurrency` is available? No, we need to pass it or define it.
// Let's create a new component `PropostaApresentacaoPrint.tsx`
const componentCode = `import React from 'react';

export default function PropostaApresentacaoPrint({ proposta }: { proposta: any }) {
  const formatCurrency = (val: number) => new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(val || 0);
  
  // Note: some variables from old FPV like 'margemBrutaMensal', 'totalBeneficiosSubtotal' were used in the DRE (Slide 10)
  // We need to ensure we don't crash. If they are used, we can default them to 0.
...
console.log('Created components/PropostaApresentacaoPrint.tsx');
