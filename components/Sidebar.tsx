...
const Sidebar = () => {
  ...
  // Audio Recording States for Desktop Chat
  ...
  // Audio Recording Functions for Desktop Chat
  ...
  const menuItems = [
    ...
    { icon: CheckSquare, label: 'Gestão de Tarefas', href: '/tasks', roles: ['ADMIN', 'MANAGER', 'USER'] },
    ...
    { icon: ClipboardList, label: 'Atas de Reunião', href: '/atas', roles: ['ADMIN', 'MANAGER', 'USER'] },
    { icon: ClipboardCheck, label: 'PIC Implantação', href: '/pic', roles: ['ADMIN', 'MANAGER', 'USER'] },
    ...
    { icon: Wrench, label: 'Área do Técnico', href: '/ativos/tecnico', roles: ['ADMIN', 'MANAGER', 'USER', 'LOGISTICA'] },
    { icon: Truck, label: 'Gestão de Entregas', href: '/entrega', roles: ['ADMIN', 'MANAGER', 'USER', 'LOGISTICA'] },
    { icon: Smartphone, label: 'Área do Entregador', href: '/entrega/entregador', roles: ['ADMIN', 'MANAGER', 'USER', 'LOGISTICA'] },
        ...
        <>
          ...
          <div className="fixed top-0 right-0 h-screen w-12 bg-white border-l border-slate-200/80 z-[170] flex flex-col py-5 items-center font-sans shadow-lg select-none no-print">
            ...
            <div className="flex flex-col items-center gap-4 w-full">
              ...
              {/* Divisor */}
              <div className="w-8 h-px bg-slate-200/80 my-1 shrink-0" />
                      ...
                      >
                        {u.avatarUrl ? (
                          <img
                            src={u.avatarUrl}
                            alt={u.nome}
                            className="w-8 h-8 rounded-full border border-slate-200/80 group-hover:border-slate-400 object-cover shadow-sm transition-all"
                          />
                        ) : (
                          <div className="w-8 h-8 rounded-full bg-slate-100 text-slate-700 border border-slate-200/80 flex items-center justify-center text-[10px] font-black uppercase transition-all group-hover:bg-slate-200/80">
                            {initials}
                          </div>
                        )}
                              ...
                              return (
                                ...
                                >
                                  ...
                                  >
                                    ...
                                    {/* Only show text if it's not a standalone image/file/audio preview */}
                                    {msg.content && (!msg.fileUrl || !(msg.fileType?.startsWith('image/') || /\.(jpg|jpeg|png|webp|gif)$/i.test(msg.fileUrl) || msg.fileType?.startsWith('audio/') || /\.(wav|mp3|ogg|webm|m4a)$/i.test(msg.fileUrl))) && (
