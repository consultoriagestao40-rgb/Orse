...
console.log("📖 Lendo o arquivo page.tsx...");

// ----------------------------------------------------
// 1. CORREÇÃO DA OCULTAÇÃO DO SLIDE DECK DE IMPRESSÃO
// ----------------------------------------------------
const oldPrintDeckOpen = '<div className="print-slide-deck">';
const newPrintDeckOpen = '<div className="print-slide-deck hidden print:block">';

if (content.includes(oldPrintDeckOpen)) {
   content = content.replace(oldPrintDeckOpen, newPrintDeckOpen);
   console.log("✔ Div print-slide-deck atualizada com 'hidden print:block' com sucesso!");
} else {
   console.log("⚠ print-slide-deck já estava corrigida ou não foi encontrada.");
}

// ----------------------------------------------------
// 2. SUBSTITUIÇÃO DO SLIDE 05 DE TELA
// ----------------------------------------------------
const screenSlideStartStr = '{/* SLIDE 05 (PRINCIPAIS SERVIÇOS PRESTADOS - COM AS 5 CATEGORIAS DE SERVIÇOS) */}';
const screenSlideEndStr = '{/* SLIDE 06 (SETORES ATENDIDOS - COM OS ICONS E COLUNAS DE INDUSTRIA E VAREJO) */}';

const screenSlideStartIdx = content.indexOf(screenSlideStartStr);
const screenSlideEndIdx = content.indexOf(screenSlideEndStr);

if (screenSlideStartIdx !== -1 && screenSlideEndIdx !== -1) {
   console.log("✔ Bloco do Slide 05 de Tela localizado!");
   
   const newScreenSlideContent = `${screenSlideStartStr}
                         {currentSlide === 5 && (
   ...
   content = content.substring(0, screenSlideStartIdx) + newScreenSlideContent + content.substring(screenSlideEndIdx);
   console.log("✔ Slide 05 de Tela substituído e atualizado!");
} else {
   console.log("❌ ERRO: Delimitadores do Slide 05 de Tela não encontrados!");
}

// ----------------------------------------------------
// 3. SUBSTITUIÇÃO DO SLIDE 05 DE PRINT
// ----------------------------------------------------
const printSlideStartStr = '{/* SLIDE 05 PRINT - PRINCIPAIS SERVIÇOS PRESTADOS */}';
const printSlideEndStr = '{/* SLIDE 06 PRINT - SETORES ATENDIDOS */}';

const printSlideStartIdx = content.indexOf(printSlideStartStr);
const printSlideEndIdx = content.indexOf(printSlideEndStr);

if (printSlideStartIdx !== -1 && printSlideEndIdx !== -1) {
   console.log("✔ Bloco do Slide 05 de Impressão (PRINT) localizado!");

   const newPrintSlideContent = `${printSlideStartStr}
   ...
   content = content.substring(0, printSlideStartIdx) + newPrintSlideContent + content.substring(printSlideEndIdx);
   console.log("✔ Slide 05 de Impressão (PRINT) substituído e atualizado!");
} else {
   console.log("❌ ERRO: Delimitadores do Slide 05 de Impressão (PRINT) não encontrados!");
}

// ----------------------------------------------------
// 4. ESCRITA DOS DADOS DE VOLTA
// ----------------------------------------------------
fs.writeFileSync(pagePath, content, 'utf8');
console.log("✔ Arquivo page.tsx atualizado com sucesso e 100% gravado!");
