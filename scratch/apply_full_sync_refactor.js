...
console.log("📖 Lendo page.tsx resetado para aplicação cirúrgica premium sincronizada...");
...
// ==========================================
// 2. REFATORAÇÃO DOS SLIDES DO PDF (IMPRESSÃO)
// ==========================================

const s03PrintStart = '{/* SLIDE 03 PRINT - NOSSA PRESENÇA / QUEM SOMOS */}';
const s07PrintStart = '{/* SLIDE 07 PRINT - PRINCIPAIS FERRAMENTAS */}';

const idx03Print = content.indexOf(s03PrintStart);
const idx07Print = content.indexOf(s07PrintStart);

if (idx03Print !== -1 && idx07Print !== -1) {
  console.log("✔ Âncoras do Bloco Institucional do PDF localizadas!");
  ...
  content = content.substring(0, idx03Print) + s03PrintStart + pdfInstitucionalCode + content.substring(idx07Print);
  console.log("✔ Bloco Institucional do PDF atualizado!");
}

// PDF COMERCIAIS
const s09PrintStart = '{/* SLIDE 09 PRINT - QUADRO EFETIVO */}';
const s10PrintStart = '{/* SLIDE 10 PRINT - ITENS INCLUSOS E EXCLUSÍDOS */}';
const s11PrintStart = '{/* SLIDE 11 PRINT - RESUMO DA PROPOSTA */}';
const s12PrintStart = '{/* SLIDE 12 PRINT - CONDIÇÕES DA PROPOSTA */}';
const s13PrintStart = '{/* SLIDE 13 PRINT - ACEITE */}';

const idx09Print = content.indexOf(s09PrintStart);
const idx10Print = content.indexOf(s10PrintStart);
const idx11Print = content.indexOf(s11PrintStart);
const idx12Print = content.indexOf(s12PrintStart);
const idx13Print = content.indexOf(s13PrintStart);
const idx13PrintEnd = content.lastIndexOf('</main>');

if (idx09Print !== -1 && idx10Print !== -1 && idx11Print !== -1 && idx12Print !== -1 && idx13Print !== -1 && idx13PrintEnd !== -1) {
  console.log("✔ Âncoras do Bloco Comercial do PDF localizadas!");
  ...
  content = content.substring(0, idx09Print) + s09PrintStart + pdfComercialCode + content.substring(idx13PrintEnd);
  console.log("✔ Bloco Comercial do PDF atualizado!");
}

// ==========================================
// 3. REFATORAÇÃO DOS SLIDES DA TELA (INTERATIVOS)
// ==========================================

const s03ScreenStart = '{/* SLIDE 03 (NOSSA PRESENÇA - QUEM SOMOS E COBERTURA SUL) */}';
const s07ScreenStart = '{/* SLIDE 07 (PRINCIPAIS FERRAMENTAS - DIVIDIDO LADO A LADO) */}';

const idx03Screen = content.indexOf(s03ScreenStart);
const idx07Screen = content.indexOf(s07ScreenStart);

if (idx03Screen !== -1 && idx07Screen !== -1) {
  console.log("✔ Âncoras do Bloco Institucional da TELA localizadas!");
  ...
  console.log("✔ Bloco Institucional da TELA atualizado!");
}

// ==========================================
// 4. REFATORAÇÃO DOS SLIDES COMERCIAIS DA TELA (09 A 13)
// ==========================================

const s09ScreenStart = '{/* SLIDE 04 (QUADRO EFETIVO - TABELA AUTOMÁTICA DA ABA 4) */}';
const screenComercialEndStr = '{/* CONTROLES DE NAVEGAÇÃO DOS SLIDES (PADRONIZADOS FORA DO SLIDE E DO NÚMERO) */}';

const idx09Screen = content.indexOf(s09ScreenStart);
const idxComercialScreenEnd = content.indexOf(screenComercialEndStr);

if (idx09Screen !== -1 && idxComercialScreenEnd !== -1) {
  console.log("✔ Âncoras do Bloco Comercial da TELA localizadas!");
  ...
  content = content.substring(0, idx09Screen) + s09ScreenStart + screenComercialCode + content.substring(idxComercialScreenEnd);
  console.log("✔ Bloco Comercial da TELA atualizado!");
}

fs.writeFileSync(pagePath, content, 'utf8');
console.log("✔ Sincronização magnífica e completa de TELA e PDF consolidada com absoluto sucesso!");
