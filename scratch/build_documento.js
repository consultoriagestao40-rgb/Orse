...
fs.writeFileSync('components/DocumentoA4.tsx', docComponent);
console.log('Created components/DocumentoA4.tsx');

// 2. PATCH page.tsx
let content = fs.readFileSync('app/propostas/nova/page.tsx', 'utf8');

if (!content.includes('import DocumentoA4')) {
...
if (!content.includes("viewMode === 'document'")) {
  content = content.replace(
    /<div>\s*<h2 className="text-base font-black text-slate-800 tracking-tight flex items-center gap-2">\s*<Presentation className="text-\[#10B981\]" size=\{18\} \/> Proposta Comercial \(Slide Deck\)\s*<\/h2>\s*<p className="text-xs text-slate-400 mt-1 font-medium">Visualize, edite e exporte a proposta comercial em formato de slides de apresentação\.<\/p>\s*<\/div>/,
    controlsUI
  );
}

// CONDITIONAL RENDER
if (!content.includes("<DocumentoA4")) {
  content = content.replace(
    /\{\/\* CONTAINER DOS SLIDES PARA VISUALIZAÇÃO EM TELA \*\/\}/g,
    `{/* CONTAINER DOS SLIDES PARA VISUALIZAÇÃO EM TELA */}
                  {viewMode === 'document' ? (
                     <div className="w-full bg-slate-200 border border-slate-300 rounded-xl overflow-hidden min-h-[600px]">
                        <DocumentoA4 
                           proposta={proposta} 
...
`
  ...
  content = content.replace(
    /\{\/\* SLIDE 13 PRINT - ACEITE \*\/\}[\s\S]*?<\/div>\s*<\/div>/,
    match => match + "\n                  )}"
  );
}
