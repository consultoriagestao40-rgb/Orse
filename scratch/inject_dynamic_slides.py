...
target_file = os.path.join(os.path.dirname(__file__), '..', 'components', 'PropostaApresentacao.tsx')
with open(target_file, 'r', encoding='utf-8') as f:
    content = f.read()

# 1. Inserir a detecção de slides dinâmicos e a função replaceTags
slide_state_placeholder = '  const [currentSlide, setCurrentSlide] = useState(1);'
replacement_state = """  const rawSlides = proposta.cliente?.clausulasA4 || [];
...
# 2. Substituir os limites de botões voltar/avançar
content = content.replace('currentSlide === 1 ? 13', 'currentSlide === 1 ? totalSlides')
content = content.replace('currentSlide === 13 ? 1', 'currentSlide === totalSlides ? 1')

# 3. Inserir a renderização do bloco dinâmico ou estático
target_div = """                     <div className={presentationMode
                        ...
                        {hasDynamicSlides ? (
                           (() => {
                              ...
                              if (layout === 'cobertura') {
                                 ...
                                 return (
                                    <div className="absolute inset-0 w-full h-full flex flex-col justify-between p-16 z-10 text-white overflow-hidden bg-slate-950 animate-fadeIn">
                                       <img 
                                          src={finalBgImage} 
                                          alt="Capa Fundo" 
                              ...
                              if (layout === 'valores') {
                                 return (
                                    <div className="absolute inset-0 w-full h-full flex flex-col justify-between p-16 z-10 animate-fadeIn bg-white text-slate-800 rounded-2xl border border-slate-200 select-none">
                              ...
                              if (layout === 'tabela') {
                                 ...
                                 return (
                                    <div className="absolute inset-0 w-full h-full flex flex-col justify-between p-12 z-10 animate-fadeIn bg-white text-slate-800 rounded-2xl border border-slate-200 select-none">
                              ...
                              if (layout === 'aceite') {
                                 return (
                                    <div className="absolute inset-0 w-full h-full flex flex-col justify-between p-12 z-10 animate-fadeIn bg-[#1e4480] text-white rounded-2xl border border-slate-200 select-none">
...
# Localizar o fechamento de slide 13 por índice de chaves
slide13_start = content.find('currentSlide === 13')
if slide13_start == -1:
    print("Slide 13 not found!")
    exit(1)

slide13_end = content.find(')}', slide13_start)
if slide13_end == -1:
    print("Slide 13 closing bracket not found!")
    exit(1)

# Inserir o fechamento do hasDynamicSlides logo após o encerramento do slide 13
content = content[:slide13_end + 2] + '\n                        )}' + content[slide13_end + 2:]

with open(target_file, 'w', encoding='utf-8') as f:
    f.write(content)

print("PropostaApresentacao.tsx successfully modified programmatically!")
