...
// Extrai o conteúdo do arquivo puramente em UTF-8 direto do Git
const originalFpv = execSync('git show 5362aa7:app/propostas/nova/page.tsx', { encoding: 'utf8' });

// Agora procuramos a string "print-slide-deck hidden"
const lines = originalFpv.split('\n');
let startIdx = -1;
let endIdx = -1;

for (let i = 0; i < lines.length; i++) {
  if (lines[i].includes('print-slide-deck hidden')) {
    startIdx = i;
  }
  if (startIdx !== -1 && lines[i].includes('</div> // Fim do Main Content')) {
    // A div fecha na linha anterior
    endIdx = i - 1;
    break;
  }
}
...
if (startIdx === -1 || endIdx === -1) {
    console.error("Não encontrou o bloco:", startIdx, endIdx);
    process.exit(1);
}

// Em app/propostas/nova/page.tsx no commit 5362aa7, havia uma div que fechava o deck.
// Vamos verificar o que fecha a div do deck lá.
// Normalmente é:
//                        </div>
//                     </div>
// 
...
// É mais seguro simplesmente encontrar a linha final:
for (let i = lines.length - 1; i >= 0; i--) {
   if (lines[i].includes('</main>')) {
      endIdx = i - 2; // Pega até a div de fechamento do deck
      break;
   }
}

let printDeck = lines.slice(startIdx, endIdx + 1).join('\n');

// Substituir as variáveis do FPV antigo para não dar erro
// O FPV antigo usava \`viewMode === 'slide'\`
printDeck = printDeck.replace(/className=\{`print-slide-deck hidden \$\{viewMode === 'slide' \? 'print:block' : ''\}`\}/g, 'className="print-slide-deck hidden print:block"');
...
console.log('Successfully recreated components/PropostaApresentacaoPrint.tsx with pristine UTF-8 encoding directly from git');
